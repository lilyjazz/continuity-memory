...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        default=str(ROOT / "reports" / "openclaw_remote_behavioral_reset_ab_results.json"),
    )
    ...
    parser.add_argument(
        "--followup-prefix",
        default="It is the next day and we resumed after reset. Continue based on prior known decisions.",
    )
...
def run_case(case: dict, gateway, service, followup_prefix: str):
    ...
    control_day1_sid = f"ab-control-r-day1-{case['case_id']}-{uuid.uuid4().hex[:8]}"
    control_day2_sid = f"ab-control-r-day2-{case['case_id']}-{uuid.uuid4().hex[:8]}"
    experiment_day1_sid = f"ab-exp-r-day1-{case['case_id']}-{uuid.uuid4().hex[:8]}"
    experiment_day2_sid = f"ab-exp-r-day2-{case['case_id']}-{uuid.uuid4().hex[:8]}"
    conversation_id = f"default:behavior-reset-{case['case_id']}-{uuid.uuid4().hex[:6]}"

    adapter = OpenClawContinuityAdapter(service=service, gateway=gateway, session_prefix="behavior-reset")
    ...
    pre_reset_update = adapter.prepare_for_compaction(conversation_id)
    ...
    reset_control_reply = gateway.ask(control_day1_sid, "/reset")
    reset_experiment_reply = gateway.ask(experiment_day1_sid, "/reset")

    day_switch_note = "This is a brand new next-day session after reset."
    ...
    for query in case["queries"]:
        followup_query = (
            f"{followup_prefix}\n"
            "We had a reset event yesterday and this is a fresh session today.\n"
            f"Question: {query['q']}\n"
    ...
    return {
        ...
        "pre_reset_anchor_version": pre_reset_update.anchor_version,
        "reset_control_reply": reset_control_reply,
        "reset_experiment_reply": reset_experiment_reply,
        "rows": rows,
    }
...
def main() -> None:
    ...
    for case in cases:
        ...
        for row in report["rows"]:
            total_queries += 1
            control_hits += int(row["control_hit_strict"])
            experiment_hits += int(row["experiment_hit_strict"])
            control_hits_semantic += int(row["control_hit_semantic"])
            experiment_hits_semantic += int(row["experiment_hit_semantic"])

    result = {
        "mode": "openclaw-remote-real-reset-behavioral-day-switch",
        "host": args.ec2_host,
        ...
        "disclosure": "Behavioral simulation with real /reset and explicit next-day new session switch. Dual-track strict+semantic evaluation enabled.",
